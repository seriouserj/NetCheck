...
Changelog: Separate report titles from full-width tables with a compact spacer.
"""
...
def report_as_html(title: str, headers: tuple[str, ...], rows: tuple[tuple[str, ...], ...]) -> str:
    ...
    return (
        ...
        "h1{font-size:16pt;color:#05285a;margin:0;}"
        ".title-gap{font-size:5pt;margin:0;}"
        ...
        "</style></head><body>"
        f"<h1>{escape(title)}</h1><p class='title-gap'>&nbsp;</p>"
